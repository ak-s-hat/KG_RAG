...
2. [Example Test Result](#example-test-result)
3. [Part 2 — The Testing Setup](#part-2--the-testing-setup)
...
4. [Tuning the Pipeline](#tuning-the-pipeline)
...
To demonstrate the system in action, we evaluated the pipeline using a real-world research paper as our knowledge base:

> **Knowledge Base:** [Long-Term Power Load Forecasting Using LSTM-Informer with Ensemble Learning](https://www.mdpi.com/2079-9292/12/10/2175) (*Electronics* 2023, 12, 2175).

### Test Case: Hybrid LSTM-Informer Architecture

| Field | Content |
|-------|---------|
| **Input Query** | *Analyze the hybrid LSTM-Informer architecture's dual mechanism for short-term event correlation and long-term dependence in power load forecasting.* |
| **Actual Output** | The architecture combines LSTM (for short-term event correlation via gate control) and Informer (for long-term dependence via self-attention mechanisms). [Full answer generated by Groq...] |

### Evaluation Metrics

- **✅ Answer Relevancy (1.0/1.0):** The answer perfectly addressed the query without including irrelevant details.
- **✅ Faithfulness (1.0/1.0):** Every claim in the generated answer was directly supported by the retrieved context.
- **⚠️ Contextual Relevancy (0.61/1.0):** 

> [!NOTE]
> **Why is Contextual Relevancy lower?**
> A score of **0.61** indicates that while the retrieved chunks were useful, there were a significant number of extra/unnecessary chunks being pulled into the context. 
> 
> This is a direct consequence of the **NER Over-extraction Problem**: Because the generic spaCy model extracts a large volume of keywords (some too generic), the Neo4j graph becomes denser than ideal. During retrieval, this causes "neighboring" chunks to be pulled in that might be about related metadata (variable selection, performance metrics) rather than the core mechanism being queried. 
> 
> However, a 61% relevance score is still considered successful as the LLM was able to filter through the noise to produce a 100% faithful and relevant answer. This confirms the efficacy of the **Fallback + LLM-as-a-Filter** strategy used in our query phase.
